...
namespace SecureChatClient
{
    public partial class Form1 : Form
    {
            ...
            try
            {
                ...
                Func<int> delUpdate = delegate ()
                {
                    if (strMessage != null && strMessage.Length > 4 && strMessage.Substring(0, 4) == "KEY:")
                    {
                        ...
                        txtEncryptKey.Text = strMessage.Substring(4) + "PrivateKeyEncryptionHash";
                    }
        ...
        private void btnGenerateKey_Click(object sender, EventArgs e)
        {
            ...
            {
                if (objPublicKey.isPrimeCheck(Convert.ToInt32(txtFactor1.Text)) && objPublicKey.isPrimeCheck(Convert.ToInt32(txtFactor2.Text)))
                {
                    ...
                    txtDecryptKey.Text = objPublicKey.d[0].ToString() + "PubilKeyTextSaltBitsAdded";
                ...
                {
                    MessageBox.Show("Please enter only prime numbers.");
                    txtFactor1.Text = string.Empty;
                    txtFactor2.Text = string.Empty;
                }
            }
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtMessage.Text != String.Empty)
                {
                    ...
                    foreach (long i in objPublicKey.strEncryptMsg)
                    {
                        if (strSend == string.Empty)
                            strSend = i.ToString();
                        else
                            strSend = strSend + " " + i.ToString();
                    }
                    // Send messages to the server
                    swClientSender.WriteLine(strSend);
                    swClientSender.Flush();
                }
    ...
    class PublicKey
    {
        ...
        public bool isPrimeCheck(long primeCheck)
        {
            int i;
        ...
        public long isPrime(long primeCheck)
        {
            int i;
            long sqrt_prime = (long)Math.Sqrt(primeCheck);
            for (i = 2; i <= sqrt_prime; i++)
